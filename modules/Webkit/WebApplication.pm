...
sub get_template
{
	...
	foreach my $key (keys %{$self->{dynamic_template_variables}})
	{
		$props->{$key} = $self->{dynamic_template_variables}->{$key};
	}
...
sub get_cookie
{
	...
	my $j = Apache2::Cookie::Jar->new($self->{_r});
	my $c = $j->cookies($key);
	
	#my %cookiejar = Apache2::Cookie->new($self->{_r})->parse;
...
sub bake_cookie
{
	...
	my $cookie = Apache2::Cookie->new($self->{_r},
		-name => $key,
		-value => $value,
		-expires => $expires,
		-path => '/' );
...
sub get_uri_file_path
{
	...
	my $document_root = $self->{_r}->document_root;
